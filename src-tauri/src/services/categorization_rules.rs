...
pub fn apply_to_uncategorized(conn: &Connection, account_id: Option<i64>) -> rusqlite::Result<usize> {
    ...
    for txn in transactions {
        let effects = apply_to_transaction(
            conn,
            txn.account_id,
            &txn.date,
            txn.amount_cents,
...
mod tests {
    ...
    fn description_contains_matches_case_insensitively() {
        ...
        let result = apply_to_transaction(&conn, 1, "2026-08-01", -500, "Whole Foods Market #42", None)
            .expect("apply rule");
    ...
    fn description_equals_requires_exact_match() {
        ...
        let no_match = apply_to_transaction(&conn, 1, "2026-08-01", -500, "Rent payment", None).expect("apply rule");
        let exact_match = apply_to_transaction(&conn, 1, "2026-08-01", -500, "rent", None).expect("apply rule");
    ...
    fn amount_field_matches_on_stringified_cents_equality() {
        ...
        let matches = apply_to_transaction(&conn, 1, "2026-08-01", -1599, "Streaming Co", None).expect("apply rule");
        let no_match = apply_to_transaction(&conn, 1, "2026-08-01", -1600, "Streaming Co", None).expect("apply rule");
    ...
    fn account_field_matches_on_account_id_equality() {
        ...
        let matches = apply_to_transaction(&conn, 7, "2026-08-01", -500, "Anything", None).expect("apply rule");
        let no_match = apply_to_transaction(&conn, 8, "2026-08-01", -500, "Anything", None).expect("apply rule");
    ...
    #[test]
    fn apply_to_transaction_prefers_merchant_name_over_raw_description() {
        let conn = db::open_in_memory().expect("open in-memory test database");
        let category_id = create_test_category(&conn, "Coffee");
        create(&conn, RuleField::Description, MatchType::Equals, "blue bottle coffee", category_only(category_id), 0)
            .expect("create rule");

        // The rule only matches the clean merchant name, not the raw
        // processor-prefixed description.
        let via_merchant = apply_to_transaction(
            &conn,
            1,
            "2026-08-01",
            -500,
        ...
        .expect("apply rule");
        let without_merchant = apply_to_transaction(
            &conn,
            1,
            "2026-08-01",
            -500,
        ...
        assert_eq!(via_merchant.category_id, Some(category_id));
        assert_eq!(without_merchant.category_id, None);
    }
    ...
    fn no_matching_rule_returns_no_effects() {
        ...
        let result = apply_to_transaction(&conn, 1, "2026-08-01", -500, "Movie Theater", None).expect("apply rule");
    ...
    fn when_multiple_rules_match_the_lowest_priority_wins() {
        ...
        let result = apply_to_transaction(&conn, 1, "2026-08-01", -500, "Coffee Shop", None).expect("apply rule");
    ...
    fn a_matching_rename_rule_produces_the_rename_value_and_no_category() {
        ...
        let effects = apply_to_transaction(&conn, 1, "2026-08-01", -500, "SBUX #1234", None).expect("apply rule");
    ...
    fn a_matching_hide_rule_produces_hide_true_with_no_category() {
        ...
        let effects = apply_to_transaction(&conn, 1, "2026-08-01", -300, "ATM Fee", None).expect("apply rule");
    ...
    fn a_matching_tag_rule_produces_its_tag_ids_with_no_category() {
        ...
        let effects = apply_to_transaction(&conn, 1, "2026-08-01", -1200, "Uber Trip", None).expect("apply rule");

...
- **Reserved Shortcut Set**: a closed, app-wide vocabulary of macOS-native modifier shortcuts, with Ctrl-equivalent fallbacks for Windows/Linux. No screen may repurpose one of these for a different action. Concrete bindings shipped:
  - **Cmd/Ctrl+N** — create the record appropriate to the current screen. Wired where a "create new record" action already existed: Accounts (opens Add Account) and Transactions (opens a new New Transaction panel, added alongside this work since the all-Accounts Transactions view had no manual-entry affordance at all — see `AllTransactionsScreen.tsx`). Not wired on screens with no such concept (Dashboard, Reports, Budget, Settings, Import) or where wiring it was out of this issue's scope (Goals/Holdings/Investments/Categories/Rules/Recurring/Merchants already have their own "Add"/"New" buttons but weren't touched by #78 — a natural follow-up, not a gap in the reserved set itself).
  ...
  - **Delete/Backspace** — delete the current selection, wired only where an unambiguous "current selection" already exists: the Transactions grid's checkbox multi-select, and only when exactly one row is selected (multi-select firing the existing per-row confirm dialog once per row would stack N confirmation prompts, which #78 didn't sign up to redesign). Every other screen's delete action is a per-row button click with no selection concept to hang a keyboard shortcut on, so it's left unwired rather than inventing one.
